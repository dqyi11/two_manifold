...
% RUN Instrumental Eigenmap ALGORITHM
[iXa, iYa]=instrumental_eigenmap(Xa', Ya', K,d);

figure;
scatter(iXa(:,1),iXa(:,2),30,color(1:N),'o');
title('Recovered 2D Linear Manifold X by IE');

figure;
scatter(iYa(:,1),iYa(:,2),30,color(1:N),'o');
title('Recovered 2D Linear Manifold Y by IE');